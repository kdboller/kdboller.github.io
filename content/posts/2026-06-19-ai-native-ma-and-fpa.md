...
While this is covered quite extensively by [McKinsey](https://www.mckinsey.com/capabilities/business-building/our-insights/the-seven-operating-truths-of-ai-native-companies?stcr=2195024BD0DE435B9CD22901DDAE02BE&cid=mgp_opr-eml-alt-dna_lp-mgp-glb--&hlkid=6396044309804a0b8890bbfc9a6918aa&hdpid=7901c200-89e6-40a8-b2f8-c406df93335a) and others, my summary view is that **most enterprise AI implementations are point solutions with a strategy-enforced ceiling**. Layering AI on top of sub-optimal workflows, or ones that should have never existed or should no longer exist, is an obvious path to failed AI implementation outcomes.

In order to succeed, before any AI implementation starts, all companies should step back and:

1. Outline a before and after state of their workflows that will be continued, added, or deprecated in order to drive to their end objectives,
2. Establish the appropriate new departmental structures and the future job responsibilities that drive the success of these new structures, and then
3. Determine the objectives they'll work backwards from.

With this organizational re-structure established, to be successful in the AI-native pursuit and to lay the foundation for ROI, an organization must then outline the software and technical foundation for enterprise-grade AI decisioning. Complexities here include picking the right AI stack (more on this further below), defining a department by department approach to **compute allocation**, and building a continually evolving revenue and cost forecasting that incorporates an organization LLM model routing strategy for all agentic AI workflows.
...
If the vast majority of vibe coded apps should largely not be in the 1% of tokens that make their way into production, what's the right framing that allows departments to leverage their organization's AI-driven decision engine?
...
* Vibe-coded apps should largely be the 99% of tokens that never make it to Production.
  * These are helpful for prototyping. For example, you can now collapse what was once a very detailed product requirements document (PRD) into a paragraph of text and a vibe coded prototype app.
* By contrast, **enterprise-grade harnesses** ensure that the 1% of token consumption is auditable, governable, and safe to deploy at enterprise scale.
...
With this underlying business foundation, Summation is an AI-native platform that is structured to transform how enterprises make decisions. It aims to accomplish this by unifying fragmented data, orchestrating AI agents at scale, and enforcing traceable verification, so that leaders have confidence to make decisions based on their verified data (deterministic outputs) and detailed insights regarding the impact that key levers are having on recent and future expected performance.
...
Last, and perhaps most critically, in order to capture the full opportunity that AI presents, **companies must wisely determine where to place their bets in terms of technical infrastructure and software**. Focusing on core competencies and where expertise lies is now more critical than ever. On the one hand, Company A could move off of Salesforce and build a vibe-coded CRM product that is non-core to its DNA and expertise, albeit now arguably possible given the unlock that is Claude Code/Codex et al. However, Company A will be on the hook for all future support, maintenance, security, and all of the related challenges that come with enterprise software. Separately, Company B could choose to build all of its reporting in bespoke artifacts built off of a Claude Code foundation. However, similar to Company A, Company B is now on the hook for all maintenance, model drift tracking, and potential security risks.

My straightforward advice to Company A and Company B would be the following: give a thorough look at Sigma and Summation, respectively. 99% of token consumption should not go to enterprise grade decision making, and the benefits of selecting platforms like these is the same as it has always been in "build versus buy" decisioning: all of the investment in vendor innovations can accrue to the benefit of enterprise customers and the learning these vendors apply to their platforms is based on all of their customers, way more signal than an individual business will get from its usage and business cases. Plus, as seen in the first 6 months of this year, the advancement cycle in these platforms is only accelerating. In terms of risk mitigation, I'd argue that software lock-in is lower than it has ever been and the ability to transition from one platform to another has never been easier due the facilitation of coding agents such as Claude Code. As a direct example, [Sigma's Tableau to Sigma repo](https://github.com/twells89/sigma-migration-skills/tree/main/plugins/tableau-to-sigma).

**Last, when enterprise AI implementation is done successfully, which is non-trivial per the details in this post, my hope is that this will also justify additional knowledge worker hires who can further growth opportunities across the organization.** We can all do more to make [Jevons' paradox](https://en.wikipedia.org/wiki/Jevons_paradox) the great reality!
